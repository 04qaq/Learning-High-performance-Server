...
// ---------------- LogEvent ----------------
// 表示一次日志事件，包含用于格式化的信息和流式写入 stringstream
...
// 构造函数：解析 pattern（默认 pattern 在 header 中给出）
LogFormatter::LogFormatter(const std::string &pattern) :
    m_pattern(pattern) {
    init();
}

// format: 根据已解析的 m_items 对 event 进行格式化
...
void StdoutLogAppender::log(LogLevel::Level level, std::shared_ptr<LogEvent> event) {
    ...
    // 先拿 formatter 副本（保护由 appender 内部 mutex 完成）
    LogFormatter::ptr f = getFormatter();
    if (!f && event) {
        static LogFormatter::ptr s_def = std::make_shared<LogFormatter>();
        f = s_def;
    }

    // 控制台输出需要保证行不混杂，故在 appender 层互斥写入
    {
...
void FileoutAppender::log(LogLevel::Level level, std::shared_ptr<LogEvent> event) {
    ...
    // 注意：可以考虑 flush 策略（按行 flush、定时 flush、或不 flush 由后台线程处理）
}
...
// log: 将 event 发送到所有 appender，线程安全（读多写少）
void Logger::log(LogLevel::Level level, std::shared_ptr<LogEvent> event) {
    if (level < getLevel()) return;

    // 1) 读锁保护读取 appenders（读多写少），并复制到本地，尽快释放锁
    std::shared_lock<std::shared_mutex> sl(m_mutex);
    auto appenders = m_appenders;        // 复制共享指针容器（快）
    auto logger_formatter = m_formatter; // 复制默认 formatter
    sl.unlock();

    // 2) 在没有持有 logger 的情况下对每个 appender 做输出（appender 内部自己负责同步）
    for (auto &app : appenders) {
        if (!app) continue;
        if (!app->getFormatter()) {
            app->setFormatter(logger_formatter);
        }
        app->log(level, event);
    }
}
...
// ---------------- LogManager ----------------
// 全局管理器构造：创建 root 并默认加一个 stdout appender
...
// getLogger: 先尝试读锁快速查找，若不存在再升级为写锁创建
Logger::ptr LogManager::getLogger(const std::string &name) {
    // 先用 shared_lock 做一次快速查找（读多写少）
    {
        std::shared_lock<std::shared_mutex> sl(m_mutex);
        auto it = m_loggers.find(name);
        if (it != m_loggers.end()) return it->second;
    }
    // 如果没找到，升级为 unique_lock 并再检查一次（避免竞争）
    {
        std::unique_lock<std::shared_mutex> ul(m_mutex);
        auto it = m_loggers.find(name);
        if (it != m_loggers.end()) return it->second;
